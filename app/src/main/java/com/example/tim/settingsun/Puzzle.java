...
public class Puzzle {
    ...
    private boolean addBlock(int x, int y, int type) {
        ...
        if (test) {
            puzzle[x][y] = type;
            for (int i = 1; i < BlockInfo.getDimensions(type).x; i++) {
                for (int j = 1; j < BlockInfo.getDimensions(type).y; j++) {
                    puzzle[x + i][y + j] = -1;
                }
            }
        }
    ...
    private boolean canMove(int x, int y, Direction d){
        ...
        //check of botsing
        if (d.dx > 0) {
            if (puzzle[x + width + d.dx][y] != 0)
                return false;
            if (puzzle[x + width + d.dx][y + height] != 0)
                return false;
        }
        ...
        if (d.dy > 0) {
            if (puzzle[x][y + height + d.dy] != 0)
                return false;
            if (puzzle[x + width][y + height + d.dy] != 0)
                return false;
        }
